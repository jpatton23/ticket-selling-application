...
export default function SellPage() {
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-8">
      <div className="mb-6">
        <Link href={`/events/${occurrenceId}`} className="text-purple-400 hover:text-purple-300 text-sm">
          ← Back to marketplace
        </Link>
      </div>
      ...
      <div className="bg-[#111111] rounded-xl border border-white/10 p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-1">Price per ticket (£)</label>
            <input
              type="number"
              step="0.01"
              min="0.01"
          ...
          {existingListing && (
            <button
              ...
              disabled={loading}
              className="w-full bg-red-500/10 hover:bg-red-500/20 disabled:opacity-50 text-red-400 font-semibold py-2 rounded-lg transition-colors border border-red-500/20"
            >
              Remove Listing
            </button>
          )}
